...
parser = argparse.ArgumentParser(
    description="Replace hacked coordinates by nan coordinates in gro files. To reverse insane_hack.py modifications. It will put nan as coordinates for atoms that are specified in atoms txt file"
)
...
atom_line_re = re.compile(
    r"^[\d\s]{5}.{5}.{5}[\d\s]{5}[\d\s-]{4}\.[\d]{3}[\d\s-]{4}\.[\d]{3}[\d\s-]{4}\.[\d]{3}"
)
previous_coords = ""
