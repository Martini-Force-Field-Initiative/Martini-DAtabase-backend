...
#  source $venv
#fi
insaneArgs="$insaneArgs -o $gro_out -p $top_out";
INPUT=input/input.pdb
echo "#>>$PATH"
echo ">>$INSANE_SCRIPT"
if [[ -f $INPUT ]]; then 
    echo "input pdb so use insane hack"
    echo Launch : python3 ./input/insaneHackBefore.py $INPUT output-insane-hack.pdb hacked-atoms.txt
    python3 ./input/insaneHackBefore.py $INPUT output-insane-hack.pdb hacked-atoms.txt
    ...
    echo Launch : python3 ./input/insaneHackAfter.py system.gro system-insane-hack.gro hacked-atoms.txt
    python3 ./input/insaneHackAfter.py system.gro system-insane-hack.gro hacked-atoms.txt
else
    echo "no input pdb found, building \"pure\" membrane"
...
[ -f $gro_out ] && [ -f $top_out ] && return

>&2 echo "Insane faile to build gro/top files"
>&2 echo "Failed cmd: python $INSANE_SCRIPT $insaneArgs"
>&2 echo "Reason:"
>&2 cat insane_redirect.stderr
