## GLA 2024/01 remove shebang
## MODIFIED BY Louis Béranger, 2020/04/10
# Modified: add residue index to indBB + replaced k+1 to residue index
# pdb parsing: removed .split() and use substr instead [:]
...
def read_data(struct_pdb, file_contacts, file_BB, file_OV, file_rCSU, header_lines, cols):
    ...
    for k in range(0, len(dat)):
        ...
        # indBB: atom index, x, y, z, residue index
        indBB.append([ 
            int(pdb_line[6:11]), 
    ...
    # but... they're merged in the same array... ??
...
def write_files(file_pref, sym_pairs, missAt, indBB, missRes, Natoms, nameAA, go_eps, c6c12):
    ...
    with open(file_pref + '_BB-part-def_VirtGoSites.itp','w') as f:
        ...
        for k in range(0, len(indBB)):
            s2print = "%s_%s 0.0 0.000 A 0.0 0.0 \n" % (file_pref, str(int(indBB[k][4]))) # residue index adapted due to missing residues
            f.write(s2print)
