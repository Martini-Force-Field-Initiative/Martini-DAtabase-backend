import { BaseMolecule, Molecule, StashedMolecule } from "./entities";
import logger from '../logger';
/**
 * CompleteMoleculeVersionTree - A collection of trees
 */

export default class CompleteMoleculeVersionTree {
  protected _trees: {[ff:string] : MoleculeVersionTree[]} = {}; 

  //constructor(public molecules: BaseMolecule[]){
  constructor(public molecules:BaseMolecule[]) {  
...
class MoleculeVersionTree {
  ...
  walk( callback:(arg0:MoleculeNode<BaseMolecule>)=>void ) {
    logger.debug(`[MoleculeVersionTree:walk] Starting walking @root ${this.root.content.alias}:${this.root.content.force_field}[${this.root.content.version}]`)
    MoleculeVersionTree._walk(callback, this.root)
   // callback(this.root);
   // this.root.allChildren.forEach( (child)=> MoleculeVersionTree._walk(callback, this.root) );
  }
  private static _walk(callback:(arg0:MoleculeNode<BaseMolecule>)=>void, n:MoleculeNode<BaseMolecule>) {
    logger.debug(`[MoleculeVersionTree:_walk] ${n.content.alias}:${n.content.force_field}[${n.content.version}] children count : ${n.children.length}`); 
    callback(n);
    n.children.forEach( (child)=> MoleculeVersionTree._walk(callback, child) );
  }
}
