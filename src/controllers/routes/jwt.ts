...
import { getUserFromToken } from "../../helpers/database/token";
...
export default expressjwt({
  ...
  isRevoked: async (req: any, packet: any) => {
    try {
      ...
      if (!user) {
        // Token is orphan !
        logger.error(
          "[expressjwt::isRevoked] Orphan token ! This should not happen. ",
          packet,
        );
        return false;
      }
  ...
  path: [
    ...
    /^\/api\/polymer_generator\/thumbnails\/.+$/,
    "/api/polymer_generator",
