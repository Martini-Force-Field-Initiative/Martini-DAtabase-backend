...
PdbGetterRouter.get('/:id', (req, res) => {
  (async () => {
    ...
    logger.debug(`/molecule/representation/${req.params.id}`);
    // File does not exists or does not have a pdb file attached
    if (!molecule || !molecule.itp.length) {
      if(!molecule) {
        logger.error(`[PdbGetterRouter:get]\"${req.params.id}\" does not exists in database`);
      } else {
        logger.error(`[PdbGetterRouter:get]\"${req.params.id} Itp seems to be missing`)
      }
    ...
    logger.debug(`[PdbGetterRouter:get] molecule_data ${inspect(molecule_data)}`);
    if (!molecule_data) {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      zip.on('error', (e:any)=> { logger.error(`[PdbGetterRouter:get] Ziping PDB file failed @${req.params.id}`);reject(e);});
    });
    ...
    logger.debug(`[PdbGetterRouter:get] Sending ${pdb_final[0]}`);
    res.json({
      radius: {},
      pdb: pdb_final[0], 
      top: top_final[0],
