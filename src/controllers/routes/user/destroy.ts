...
DestroyUserRouter.delete('/', (req, res) => {  
  (async () => {
    ...
    await Promise.all(stashed_for_user.map(s => DatabaseMoleculeDesk.deleteMolecule(s.id, req.full_user!, true)));
    await Promise.all(molecule_for_user.map(s => DatabaseMoleculeDesk.deleteMolecule(s.id, req.full_user!, false, false)));
