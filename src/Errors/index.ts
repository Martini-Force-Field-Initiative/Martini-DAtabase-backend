...
import { JobFileSystem } from "ms-jobmanager";
...
const ErrorsToText = {
  ...
  [ErrorType.MoleculeNotFound]: [
    404,
    "You can't edit a molecule that does not exists",
  ],
  ...
  [ErrorType.MissingFiles]: [
    400,
    "Missing files attached to request, at least one ITP file and one PDB file is required",
  ],
  ...
  [ErrorType.InvalidMoleculeFiles]: [
    400,
    "Sended molecule files (ITP, PDB/GRO) are incorrect",
  ],
  [ErrorType.InvalidForceField]: [400, "Unknown force field"],
  [ErrorType.InvalidVermouthLib]: [400, "Unknown Vermouth (.ff) library file"],
  [ErrorType.InvalidMartinizeVersion]: [
    400,
    "Martinize version does not exists",
  ],
  [ErrorType.InvalidUsername]: [
    400,
    "Username contains illegal characters or is less than 2 characters long",
  ],
  ...
  [ErrorType.ElasticNetworkFailed]: [
    404,
    "Computation of elastic network failed",
  ],
  [ErrorType.IncorrectItpName]: [
    400,
    "The itp file name could not be parsed, please check the syntax",
  ],
  [ErrorType.MissingTopFiles]: [
    400,
    "Missing files attached to request, there must be one top file for each itp",
  ],
  ...
  [ErrorType.HistoryFilesNotFound]: [
    404,
    "Job result files not found on distant file system",
    "HistoryFileNotFound",
  ],
  ...
  [ErrorType.ConsistencyVersionTree]: [
    400,
    "Several tree version for this molecule",
    "ConsistencyVersionTree",
  ],
};
...
export default new (class Errors {
  ...
  throw(code: ErrorType, additionnal?: object): never {
    logger.error(
      `[ApiError] throwing ${code} ${additionnal != undefined ? inspect(additionnal) : ""}`,
    );
    const [http_code, message] = ErrorsToText[code] as [number, string];
  ...
  make(code: ErrorType, additionnal?: object): ApiError {
    const [http_code, message, type] = ErrorsToText[code] as [
      number,
      string,
      string?,
    ];
...
export class JobFSError extends Error {
  jobFS: JobFileSystem;
  constructor(msg: string, jobFS: JobFileSystem) {
    super(msg);
    this.name = "JobFSError";
    this.jobFS = jobFS;
  }
}
