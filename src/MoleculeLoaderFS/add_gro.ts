...
import { MoleculeDatum } from "./infoSetter";
import StreamZip from 'node-stream-zip';
import TmpDirHelper from "../TmpDirHelper";

export const addGroFromBatch = async (batch : MoleculeDatum[]) => {
...
const addOneGro = async(molecule : MoleculeDatum, tmpDir : string) => {
    for (const version of molecule.versions) {
        ...
        console.log(version)
        const currentItpMd5 = await md5File(molecule.itp.path)
